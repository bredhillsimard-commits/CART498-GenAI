...
What didn't work: plain text-to-image kept placing the birdcage *next to* the figure, and inpainting the neck region kept producing a human head. Generating the pieces separately and pasting them is what made the swap land, and it keeps the cut edge that makes it read as collage.

## Files

- `assignment_1.ipynb`: Colab notebook with all the code (diffusers pipelines, seed sweep, outpainting, cut-and-paste)
- `surreal_collage_{1,2,3}.txt`: prompts for each image (base plate + head fragment)
- `surreal_collage_{1,2,3}.jpg`: the three collages
- `essay.pdf`: short essay on generative AI in creative fields

## Settings

| | |
|---|---|
| Text-to-image | `stabilityai/stable-diffusion-xl-base-1.0` (fp16), 1024 × 1024, 40 steps, guidance 7.5 |
| Inpainting | `diffusers/stable-diffusion-xl-1.0-inpainting-0.1` UNet (outpainting the headroom) |
| Seed | 498 for every generation |
